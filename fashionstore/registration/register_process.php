...
$sql3 = "INSERT INTO `users`VALUES (DEFAULT,'$first_name','$last_name','$email','$token',1,0,0)"; //dodawanie użytkownika
if($conn->query($sql3)) // jezeli zostanie dodany, dodajemy adres
{
    ...
    if($conn->query($sql2))
    {
       ...
       {
           if(!empty($id2)) {
               $actual_link = "http://halada.ddns.net/fashionstore/registration/active.php?id=". $id2;
               $toEmail = $email;
               $subject = "Aktywacja konta uzytkownika";
               $content = "Potwierdz rejestracje konta poprzez ten link $actual_link ";
               $mailHeaders = "From: FashionShop\r\n";
               if (mail($toEmail, $subject, $content, $mailHeaders)) {
                   $message = "Zarejestowałeś się, naciśnij link aby potwierdzić lub zignoruj.";
                   $type = "success";
               }
           }
